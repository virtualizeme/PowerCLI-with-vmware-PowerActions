param(
    ...
    [Parameter(Mandatory=$true)]
    [VMware.VimAutomation.ViCore.Types.V1.Network.Network]$Network,
...
for ($i = 1; $i -le $Count; $i++) {
    ...
    New-VM -Template $Template -Name $VMName -VMHost $VMHost -Datastore $Datastore -OSCustomizationSpec "MyCustomSpec" -Location (Get-Folder -Name "MyFolder")

    # Konfiguracja sieci dla nowej VM
    $networkAdapter = Get-NetworkAdapter -VM $VMName
    Set-NetworkAdapter -NetworkAdapter $networkAdapter -NetworkName $Network.Name -Confirm:$false
